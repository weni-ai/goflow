...
func (c *Contact) UpdatePreferredChannel(channel *Channel) bool {
	...
	if channel == nil {
		...
		for _, urn := range c.urns {
			...
			// move any URNs with this channel or assign channel to urn if same is type, to the front of the list
			if urn.Channel() == channel || channel.SupportsScheme(urn.URN().Scheme()) {
				urn.SetChannel(channel)
