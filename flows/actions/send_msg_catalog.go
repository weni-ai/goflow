...
func (a *SendMsgCatalogAction) Execute(run flows.FlowRun, step flows.Step, logModifier flows.ModifierCallback, logEvent flows.EventCallback) error {
	...
	for _, dest := range destinations {
		...
		if a.SearchType == "vtex" {
			...
			regexIntelligent := regexp.MustCompile(`^https:\/\/([a-zA-Z0-9_-]+)\.([a-zA-Z0-9_-]+)\.com(\.br)?\/api\/io\/_v\/api\/intelligent-search\/product_search\/?(?:\?(?:[^=]{1,50}=[^&]{1,500})(?:&[^=]{1,50}=[^&]{1,500}){0,10})?$`)
		...
		if a.createMsgCatalogAction.AutomaticSearch {
			...
			context = orgContext.GetExtraPromptByChannelUUID()
			var extraPrompt string
			if context != nil {
				extraPrompt = context.OrgContext.ExtraPrompt()
			}
